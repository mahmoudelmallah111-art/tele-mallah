...
# =========================================================
# 1) خادم ويب وهمي لإبقاء الخدمة شغالة مجاناً على Render
# =========================================================
...
# =========================================================
# 2) إعدادات المتغيرات الأساسية والبوت
# =========================================================
API_ID = 28513802
    ...
    if is_adm:
        if data == "admin_requests_panel":
            ...
            if target_uid in users:
                ...
                # إتاحة إعادة توجيه القائمة للآدمن بأمان بدلاً من التعيين المباشر
                users_list_data = load_users()
                buttons = []
                for uid, uinfo in users_list_data.items():
                    status_emoji = (
                        "⏳ معلق"
                        if uinfo.get("status") == "pending"
                        else ("✅ مقبول" if uinfo.get("status") == "approved" else "❌ مرفوض")
                ...
                try:
                    await event.edit(
                        "👥 **إدارة المستخدمين:**\nاختر مستخدماً لتغيير حالته أو إنهاء جلسته:",
                        buttons=buttons,
                        parse_mode="markdown",
                    )
                except Exception:
